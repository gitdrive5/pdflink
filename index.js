...
input.addEventListener("keypress", function(event) {
  if (event.key === "Enter") {
    ...
    document.getElementById("movieSubmit").click();
  }
});
...
const displayMovie = async function () {
    ...
    if (movieInput == '') {
        ...
        showAlert('Type any pdf Name or Date ')
        return false;
    }

    updateTitle(`Search Results for "${movieInput}"`)
    document.getElementById('results').innerText = 'Find Link 🔎';
    document.getElementById('count').innerText = 0
